...
// Example shows the full dbqueue lifecycle: apply Migration, start Run (it
// does nothing until then), Enqueue a job, and let Run poll, claim, and
// dispatch it. Waiting on a channel the job closes — rather than sleeping —
// keeps the example deterministic despite Run's poll loop being
// asynchronous.
func Example() {
	driver := sqlite.New()
